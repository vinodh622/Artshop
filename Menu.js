...
const Menu = ({navigation, route}) => {
  ...
  useEffect(() => {
    ...
    form.append(
      'cateid',
      route.params?.isSub === true
        ? route.params?.subCat
        : route.params?.productId.id,
    );
    form.append(
      'subcateid',
      route.params?.isSub === true ? route.params?.productId.id : '',
    );
 
    fetch(
      'http://artshop.shiftlogics.com/api/product/betaproductbyid',
      {
        method: 'POST',
        headers: new Headers({
          ...
          <View style={StylesAll.productLists}>
            <View
              ...
              {item.image.map((ee) => {
                return (
                  <Image
                    source={{
                      uri: `http://artshop.shiftlogics.com/${ee.image}`,
                    }}
            ...
            <View style={{flex: 1, paddingLeft: 10}}>
              <View>
                ...
                <Text numberOfLines={3} style={{color:"#171717"}}>{item.sku}</Text>
              </View>
              <View
                style={{
                  flexDirection: 'row',
                ...
                <Text
                  ...
                  RM {(Math.round(item.sellingPrice * 100) / 100).toFixed(2)}
                </Text>
              </View>
            </View>
          </View>
