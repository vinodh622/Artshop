...
const notificationValues = {
  ...
  const applyRefercode = () => {
    ...
    if (referralCode === '') {
      ...
      fetch(
        'http://artshop.shiftlogics.com/api/user/referral',
        {
          method: 'POST',
          headers: new Headers({
  ...
  const submitOTPAPI = async () => {
    ...
    if (currentOTP.length == 4) {
      ...
      try {
        dispatch(Ltout(purgeStoredState));
        await dispatch(
          loginPhoneAction(apiToken, currentOTP, phoneNumber,(route?.params.loginfrom == 'Login') ? 'http://artshop.shiftlogics.com/api/user/loginotpverified' : 'http://artshop.shiftlogics.com/api/user/otpverified',navigation,notificationValues),
        ).then(() => {
          setIsLoadingList(false);
        });
      } catch {}
  ...
  useEffect(() => {
    if (route?.params.loginfrom == 'Create') {
        ...
        fetch(
          'http://artshop.shiftlogics.com/api/user/sendotp',
          {
            method: 'POST',
            headers: new Headers({
              Accept: 'application/json',
  ...
  return (
    <View
      style={{flex: 1, flexDirection: 'column', backgroundColor: '#F9FCFB'}}>
     ...
     <View style={{marginBottom: 10, marginHorizontal: 20}}>
        ...
        <View
          ...
          {done == false ? (
            ...
            <TouchableOpacity onPress={() => resendOTPAPI((route?.params.loginfrom == 'Login') ? 'http://artshop.shiftlogics.com/api/user/loginphone' : 'http://artshop.shiftlogics.com/api/user/sendotp')}>
              <View
                style={[
                  {
                    padding: 10,
        ...
        <View style={{flex: 1, justifyContent: 'flex-end', margin: 20}}>
          ...
          {(route?.params.loginfrom == 'Login') ? null : <TouchableOpacity
            ...
            <Text style={{padding: 20, textAlign: 'center',fontFamily:'Roboto-Medium',color: '#9C9C9C'}}>
             {referralBtnText}
            </Text>
          </TouchableOpacity>}
